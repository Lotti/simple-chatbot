...
class Chat extends React.Component {
    ...
    shouldComponentUpdate(nextProps, nextState, nextContext) {
        if (this.props.session) {
            if (this.props.session.session_id !== nextProps.session.session_id) {
                return true;
            } else if (this.props.session.transcription !== nextProps.session.transcription) {
                return true;
            }
        }
    ...
    sendMessage = (text) => {
        ...
        if (['riavvia', 'ricomincia', 'restart', 'start', 'reset'].includes(text.toLowerCase().trim())) {
            resetSession();
        } else {
            addMessages(newMsg(this.getNextIndex(), text, {name: userName}, 'user'));
    ...
    forwardToWatson = (text) => {
        ...
        sendMessage(text).catch(error => {
                console.error('Si è verificato un errore nell\'invio del messaggio al server', error.message);
                message.error('Si è verificato un errore nell\'invio del messaggio al server. Riprovare.');
