...
class ChatWidget extends React.Component {
    ...
    render() {
        const {messages, loading, showInputArea, fixedAnswers, botName} = this.props;
        ...
        return (<React.Fragment>
            ...
            <div className="fixed-answers">
                {fixedAnswers.map((a, i) =>
                    <Tag key={'fa-' + i} className="fixed-answer" color="#004280"
            ...
            {showInputArea &&
            <TextAreaButton loading={loading}
                            autoComplete="off"
                            ...
                            onKeyDown={e => {
                                ...
                                if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
                                    if (latestMessages[latestPointer]) {
                                        this.textAreaButton.current.setValue(latestMessages[latestPointer]);
                                    } else {
                                        this.textAreaButton.current.setValue('');
                                    }
                                }
    ...
    static propTypes = {
        ...
        fixedAnswers: PropTypes.arrayOf(PropTypes.shape({
