...
class Assistant {
  constructor() {
    ...
    if (process.env.ASSISTANT_APIKEY) {
      ...
      assert(
        ...
        `Missing environment var ASSISTANT_USERNAME`
      );
      assert(
        process.env.ASSISTANT_PASSWORD,
        `Missing environment var ASSISTANT_PASSWORD`
      );
  ...
  _responseManipulation(response) {
    ...
    if (
      ...
      if (
        intent &&
        context.ItsConfidenceTreshold &&
        intent.confidence >= context.ItsConfidenceTreshold
      ) {
  ...
  async sendMessage(userId, text, extra) {
    ...
    let restart = false;
    if (
      [`restart`, `ricomincia`, `riavvia`].includes(text.toLowerCase().trim())
    ) {
      text = ``;
      restart = true;
    }
    ...
    try {
      ...
      if (extra.context) {
        if (
          extra.context._toClear &&
          Array.isArray(extra.context._toClear) &&
          extra.context._toClear.length > 0
        ) {
      ...
      return this._responseManipulation(
        (await this.service.message(options)).result
      );
    } catch (error) {
      log.error(error);
  ...
  getContextVar(response, varName, defaultValue) {
    if (
      response.context &&
      response.context.skills &&
      response.context.skills[`main skill`] &&
      response.context.skills[`main skill`].user_defined &&
      response.context.skills[`main skill`].user_defined[varName]
    ) {
      if (
        response.context.skills[`main skill`].user_defined[varName] !== null
      ) {
