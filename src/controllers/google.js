...
const formatResponse = (response, googleBody) => {
  ...
  const suggestion = response.output.generic.find(
    (g) => g.response_type === `suggestion`
  );
  if (suggestion) {
    ...
    if (capabilities.includes(`actions.capability.SCREEN_OUTPUT`)) {
      ...
      richResponse.items.push({
        simpleResponse: {
          textToSpeech: suggestion.suggestions
            .map((o) => {
              storeButtonInContext(conversationToken, o);
              return o.label.trim();
            })
    ...
    // testi
    const texts = response.output.generic
    ...
    if (capabilities.includes(`actions.capability.SCREEN_OUTPUT`)) {
      const img = response.output.generic.find(
        (g) => g.response_type === `image`
      );
      if (img) {
    ...
    // pulsanti
    const option = response.output.generic.find(
      (g) => g.response_type === `option`
    );
    if (option) {
      ...
      if (
        capabilities.includes(`actions.capability.SCREEN_OUTPUT`) &&
        simpleResponses >= maxSimpleResponses &&
        basicCard < maxBasicCard
      ) {
        basicCard++;
        richResponse.items.push({
          basicCard: {
    ...
    // link out button
    const connect_to_agent = response.output.generic.find(
      (g) => g.response_type === `connect_to_agent`
    );
    if (connect_to_agent) {
      richResponse.linkOutSuggestion = {
        destinationName: connect_to_agent.message_to_human_agent,
        url: Assistant.getContextVar(
          response,
          `_externalUrl`,
  ...
  const finalResponse = Assistant.getContextVar(
    response,
    `_finalResponse`,
    false
  );
...
const formatInput = (body) => {
  ...
  if (body.conversation.conversationToken) {
    const conversationToken = jwt.verify(
      body.conversation.conversationToken,
      jwtSecret
    );
    extra = { ...extra, context: conversationToken.context };
    if (
      conversationToken.buttons &&
      conversationToken.buttons[text.toLowerCase()]
    ) {
...
module.exports.google = async (req, res) => {
  ...
  try {
    ...
    if (input.extra.sessionEnd) {
      ...
      log.info(
        `Received message from %s: %s %j`,
        input.userId,
        input.text || `(empty)`,
        input.extra
      );
      const r = await Assistant.sendMessage(
        input.userId,
        input.text,
        input.extra
      );
