...
const {
  ...
  displayTemplate,
  quitResponse,
} = require(`../libraries/alexaTemplates`);
...
const formatResponse = (response, alexaBody) => {
  ...
  if (alexaBody && alexaBody.context && alexaBody.context.System) {
    capabilities = Object.keys(
      alexaBody.context.System.device.supportedInterfaces
    );
  ...
  if (capabilities.includes(`Alexa.Presentation.APL`)) {
    ...
    if (card) {
      output.response.card = {
        ...
        text: card.text,
        image: card.url
          ? { smallImageUrl: card.url, largeImageUrl: card.url }
          : undefined,
      };
    }
  }
...
const formatInput = (body) => {
  ...
  if (request.type === `System.ExceptionEncountered`) {
    ...
    if (
      ...
      request.intent &&
      request.intent.name === `watson` &&
      request.intent.slots &&
      request.intent.slots.speech &&
      request.intent.slots.speech.value
    ) {
  ...
  if (
    ...
    body.session.attributes.conversationToken
  ) {
